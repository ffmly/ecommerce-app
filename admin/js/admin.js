...
function loadCategories() {
    ...
    categories.forEach((category, index) => {
        if (category.name === 'All') return; // Skip the "All" category as it's not editable
        ...
        tr.innerHTML = `
            ...
            <td>${productCount}</td>
            <td>
                <button onclick="editCategory(${index})" class="btn btn-primary btn-sm">Edit</button>
                <button onclick="deleteCategory(${index})" class="btn btn-danger btn-sm">Delete</button>
            </td>
        `;
        tableBody.appendChild(tr);
    });
...
function saveCategory() {
    ...
    const existingCategory = categories.find((c, index) => 
    ...
    const categoryData = {
        id: categoryId === '' ? categories.length + 1 : categories[categoryId].id,
        name: categoryName,
    ...
    if (categoryId === '') {
        ...
        addNotification('New category added: ' + categoryData.name, 'fa-tags');
    } else {
        categories[parseInt(categoryId)] = categoryData;
        addNotification('Category updated: ' + categoryData.name, 'fa-tags');
    }
...
function deleteCategory(index) {
    ...
    if (hasProducts) {
        alert('Cannot delete category that has products. Please reassign or delete the products first.');
        return;
    }
    ...
    if (confirm('Are you sure you want to delete this category?')) {
        ...
        localStorage.setItem('categories', JSON.stringify(categories));
        addNotification('Category deleted: ' + categoryName, 'fa-trash');
        loadCategories();
        updateProductCategorySelects();
    }
}

// Update product form to use categories
...
function loadOrders() {
    ...
    orders.forEach((order, index) => {
        ...
        tr.innerHTML = `
            ...
            <td>
                <div class="fw-medium">${order.payment.total.toFixed(2)} DZD</div>
                ...
                <div class="modal-body">
                            ...
                            <tbody>
                                ${order.items.map(item => `
                                    <tr>
                                        ...
                                        <td>${item.quantity}</td>
                                        <td>${item.subtotal.toFixed(2)} DZD</td>
                                    </tr>
                                `).join('')}
                            </tbody>
                        </table>
                    ...
                    <div class="row mt-4">
                        ...
                        <div class="col-md-6">
                            ...
                            <p><strong>Subtotal:</strong> ${order.payment.subtotal.toFixed(2)} DZD</p>
                            <p><strong>Shipping:</strong> ${order.payment.shipping.toFixed(2)} DZD</p>
                            <p><strong>Total:</strong> ${order.payment.total.toFixed(2)} DZD</p>
                        </div>
                    </div>
                </div>
            </div>
                        ...
                        <table class="table">
                            ...
                            <tfoot>
                                <tr>
                                    ...
                                    <td>${order.payment.subtotal.toFixed(2)} DZD</td>
                                </tr>
                                <tr>
                                    <td colspan="4" class="text-end fw-bold">Shipping:</td>
                                    <td>${order.payment.shipping.toFixed(2)} DZD</td>
                                </tr>
                                <tr>
                                    <td colspan="4" class="text-end fw-bold">Total:</td>
                                    <td>${order.payment.total.toFixed(2)} DZD</td>
                                </tr>
                            </tfoot>
                        </table>
                    </div>
                ...
                <table class="table">
                    ...
                    <tbody>
                        ${order.items.map(item => `
                            <tr>
                                <td>${item.name}</td>
                                <td class="text-end">${item.price.toFixed(2)} DZD</td>
                                <td class="text-end">${item.quantity}</td>
                                <td class="text-end">${item.subtotal.toFixed(2)} DZD</td>
                            </tr>
                        `).join('')}
                    </tbody>
                    <tfoot>
                        <tr>
                            ...
                            <td class="text-end">${order.payment.subtotal.toFixed(2)} DZD</td>
                        </tr>
                        <tr>
                            <td colspan="3" class="text-end fw-bold">Shipping:</td>
                            <td class="text-end">${order.payment.shipping.toFixed(2)} DZD</td>
                        </tr>
                        <tr>
                            <td colspan="3" class="text-end fw-bold">Total:</td>
                            <td class="text-end fw-bold">${order.payment.total.toFixed(2)} DZD</td>
                        </tr>
                    </tfoot>
                </table>
...
function saveOrderStatus(index) {
    ...
    // Trigger storage event for profile stats update
        ...
        <div class="bg-white rounded-lg shadow p-6">
            <div class="flex items-center">
                ...
                <div class="ml-4">
                    ...
                    <p class="text-2xl font-semibold">${totalRevenue.toFixed(2)} DZD</p>
                </div>
            </div>
            <div class="mt-4">
                <p class="text-sm text-gray-500">
                    <span class="text-blue-600">
                        <i class="fas fa-arrow-up"></i> ${recentRevenue.toFixed(2)} DZD
                    </span>
